...
To answer this question, we'll need to calculate day of week from the `date` column, and use it to re-aggregate the activity data and generate two charts. From the charts we observe that weekdays appear to have a consistently higher overall level of activity throughout the daytime hours than the weekends. 

Output from the `summary()` function confirms this, because the median, mean, and 75th percentile for the weekday data are all higher than the corresponding values for weekends, even though the maximum value from weekend days is higher than the maximum on weekday days. 

<table>
<tr><th>Measure&nbsp;&nbsp;&nbsp;</th><th>Weekday</th><th>Weekend</th><tr>
...
Both weekdays and weekends show peak activity in the morning, as was described earlier in the analysis where the interval where the maximum average steps occurred was 8:35AM. By setting the y axis scales consistently across both charts, we can see that the highest activity is on weekend morning, which is confirmed by the output of the `summary()` function as we described previously. 
...
abline(h=c(100,200),lty=2,col="blue")
